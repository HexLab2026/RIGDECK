...
1. Publish `version.json` to your repo's `main` with a *higher* number than your running build,
   e.g. set `"version": "3.10"` while your exe is still 3.9.
2. Start RigDeck. Within a few seconds the PC window shows **UPDATE AVAILABLE — v3.10** and the
...
**Full test:** cut a real v3.10 release with a rebuilt exe attached, leave your local exe at 3.9,
...
3. **Tag:** type `v3.9`, click "Create new tag: v3.9 on publish".
4. **Title:** `RigDeck v3.9`.
...
(tag `v3.10`, attach the new exe), then bump `version` in `version.json` and commit it to
