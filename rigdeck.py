...
3) ETS2 keybinds  (Options -> Keys & Buttons)  — keep NUM LOCK ON:
     ...
     Hazard lights ......... Numpad /   (bind in-game; panel-owned)
...
_update = {"latest": None, "url": RELEASES_URL, "download": None, "notes": "", "outdated": False}
...
def _check_for_update():
    """Fetch version.json from GitHub and flag if a newer build exists.
    Notify-only: never downloads or replaces anything."""
    while True:
        try:
...
def _is_frozen():
    """True when running as the built .exe (PyInstaller), not python rigdeck.py."""
    return getattr(sys, "frozen", False)


def perform_self_update():
    """Download the new exe and swap it in via a helper, then relaunch.

    Only works when frozen (running as RigDeck.exe on Windows). Every step is
    guarded: download to a temp file, verify it's a real non-empty exe, back up
    the current exe, and only then hand off to a helper .bat that waits for this
    process to exit, swaps the files, and starts the new one. On any failure the
    running exe is left untouched.

    Returns (ok, message)."""
    if not _is_frozen():
        return False, "Update swap only works on the built .exe. Run the new file manually."
    if os.name != "nt":
        return False, "Self-update is Windows-only."

    with _update_lock:
        dl = _update.get("download")
        latest = _update.get("latest")
    if not dl:
        return False, "No download URL in version.json (add a \"download\" field)."

    exe_path = os.path.abspath(sys.executable)          # the running RigDeck.exe
    folder   = os.path.dirname(exe_path)
    tmp_new  = os.path.join(folder, "RigDeck.new.exe")
    backup   = os.path.join(folder, "RigDeck.old.exe")

    # 1. download to a temp name
    try:
        req = urllib.request.Request(dl, headers={"User-Agent": "RigDeck"})
        with urllib.request.urlopen(req, timeout=60) as r, open(tmp_new, "wb") as f:
            f.write(r.read())
    except Exception as e:
        try: os.remove(tmp_new)
        except Exception: pass
        return False, f"Download failed: {e}"

    # 2. sanity-check the download: non-trivial size + Windows exe header 'MZ'
    try:
        if os.path.getsize(tmp_new) < 500_000:
        ...
        return False, f"Downloaded file looks wrong ({e}). Update cancelled, nothing changed."

    # 3. write a helper .bat: wait for us to exit, swap files, relaunch
    helper = os.path.join(folder, "_rigdeck_update.bat")
    bat = (
        "@echo off\r\n"
        "setlocal\r\n"
        ":wait\r\n"
        ...
        f'move /y "{tmp_new}" "{exe_path}" >nul\r\n'
        f'start "" "{exe_path}"\r\n'
        f'del "{backup}" 2>nul\r\n'
        'del "%~f0"\r\n'
    )
    # keep a backup of the current exe first (best-effort)
    try:
        import shutil
    ...
    try:
        ...
        return False, f"Could not write updater helper: {e}"

    # 4. launch helper detached, then quit so the exe unlocks
    try:
        import subprocess
    ...
    return True, f"Updating to v{latest}\u2026 RigDeck will restart."
...
HAZ_KEY  = "numslash"     # hazard lights: bind in-game to Numpad /, panel-owned
...
def _status_window():
    ...
    upd_lbl = tk.Label(upd_frame, text="", bg="#14171B", fg="#FFB03F",
    ...
    def do_update():
        ...
        upd_status.config(text="Downloading new version\u2026", fg="#8A917A")
        def run():
            ok, msg = perform_self_update()
            try:
                upd_status.config(text=msg, fg="#59B06B" if ok else "#FF5340")
                if not ok:
                    upd_btn.config(state="normal", text="TRY UPDATE AGAIN")
            except Exception:
                pass
        threading.Thread(target=run, daemon=True).start()

    upd_btn = tk.Button(upd_frame, text="UPDATE NOW", command=do_update,
    ...
    def poll_update():
        ...
        if outdated and latest:
            upd_lbl.config(text=f"UPDATE AVAILABLE  \u2014  v{latest}")
            if not has_dl:
                upd_btn.pack_forget()
                upd_status.config(text="Open the releases page to download.", fg="#8A917A")
                upd_status.pack(pady=(4, 0))
            if not upd_frame.winfo_ismapped():
                upd_frame.pack(pady=(14, 0))
                root.geometry("360x300")
        root.after(5000, poll_update)

    root.after(2000, poll_update)
